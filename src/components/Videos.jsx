...
const [videosrc, setvideosrc] = useState({url: videosArr[0], title: 'Sample video 1'}); //iska format kuch aisa hota ki useState ke andar wo dalte hai jo apne ko pehale chaie rehta hai... aur uske aage destructuralisation ka use karte hai means const [videosrc, setvidesrc] videosrc current value of useState ka leta hai aur setvideosrc change hone wala vedio lega.

const changeVideo = (url, title) => {
    setvideosrc({ url, title });
  };

  return (
    <Stack direction={['column','row']} h={'100vh'}>
    <VStack w={'full'}>
    <video controls controlsList='nodownload' src={videosrc.url} style={{ width:"100%"}}></video>
    <VStack overflowY={"auto"} p={'8'} w={'full'} alignItems={'flex-start'}>
    ...
    <VStack w={['full','xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
    {
        ...
        <Button key={index} variant={'ghost'} colorScheme='purple' onClick={() => changeVideo(item, `Sample video ${index + 1}`)}>Lecture {index+1}  </Button>
        ))
    }
    </VStack>
    </Stack>
